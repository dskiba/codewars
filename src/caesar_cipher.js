export const caesar_cipher = (message, shift) => {
  const a = 'abcdefghijklmnopqrstuvwxyz';
  const msgArr = message.split('')
  const res = []

  return msgArr.map((letter) => {
    if (!letter.match(/^[A-Za-z]+$/)) {
      return letter
    }
    if (letter === letter.toUpperCase()) {
      let indexOfLetter = a.indexOf(letter.toLowerCase()) + shift
      if (indexOfLetter >= 26) {
        indexOfLetter -= 26

        const shiftedLetter = a[indexOfLetter]
...
// const cipher = message.replace(/[a-z]/gi, (c) => {
//   const nextIdx = a.indexOf(c) + n
//   const replaced = a[a.indexOf(c) + n]
//   return replaced
// });
// console.log({ cipher })
// return cipher
...
// fixed number of places up or down the alphabet.
//
...
// containing the coded string in 5 parts (five parts because, to avoid more risks,
...
//     of 4, 4, 4, 4, 1. The parts 1, 2, 3, 4 are evenly split and the last part of length 1 is shorter.
